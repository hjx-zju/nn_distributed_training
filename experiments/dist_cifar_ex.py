...
from models.squeeze_net import SqueezeNet,CIFAR_NET
from models.resnet import RESNET
from problems.dist_cifar_problem import DistCIFARProblem
...
def experiment(yaml_pth):
   ...
   # 测试集不需要进行数据增强
    ...
    joint_train_set = datasets.CIFAR10(
        data_dir, train=True, download=True, transform=transform_train
    )
    val_set = datasets.CIFAR10(data_dir, train=False, transform=transform)
    ...
    if exp_conf["data_split_type"] == "random":
        ...
        classes = torch.unique(torch.tensor(joint_train_set.targets))
        train_subsets = []
        if N <= len(classes):
            joint_labels = torch.tensor(joint_train_set.targets)
    ...
    elif exp_conf["data_split_type"] == "non_iid_dir":
        indices = list(range(len(joint_train_set)))
        

    # Create base model
    model_conf = exp_conf["model"]
    base_model = RESNET()
    # base_model = CIFAR_NET(3,5,64)
    # Define base loss function
    if exp_conf["loss"] == "NLL":
        base_loss = torch.nn.NLLLoss()
    elif exp_conf["loss"] == "CE":
